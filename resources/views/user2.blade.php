     ...
     <div class="container">
         ...
         <div class="collapse navbar-collapse" id="mynav">
             <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                 ...
                 <li class="nav-item">
                     <a class="nav-link" style="cursor:default"><span class="fas fa-user pe-2"></span> Hello {{$user->name}}</a>
                 </li>
                 <li class="nav-item">
                     <a class="nav-link" style=""><center>{{$followers}}<div style="border-bottom:1px solid #444;">Followers</div></center></a>
                 </li>
             </ul>
         </div>
     </div>
                ...
                <div class=''>
                  ...
                  <span>
                        ...
                        <center>
                          ...
                          <tr>
                            ...
                            <td style="text-align:right" colspan="3" rowspan="5">
                              <label for='photo'><img src="{{asset('profiles/'.$user->photo)}}" class='photo' name='photo' title='Choose photo' style="border-radius:100px;height:300px;width:300px" id='profilephoto'/></label><br>
                          ...
                          <tr>
                            <td><input id='updatename' class='form-control' type='text' value='{{$user->name}}' name='clientname' /></td>
                            <td><input id='updatephone' class='form-control' type='text' value='{{$user->phone}}' name='clientphone' /></td>
                          </tr>
                          <tr>
                            <th>My Address</th>
                            <td><input id='updateaddress' class='form-control' placeholder="My Address" type='text' value='{{$user->address}}' name='clientaddress' /></td>
                          </tr>
                          <tr>
                            <th>Card Number</th>
                            <td>
                              <input type="text" class="form-control" id="cardNumber" maxlength="19" value="{{$user->card_number}}" />
                            </td>
                          </tr>
                          <tr>
                            <td colspan="2"><textarea id='updateaboutme' class='form-control' type='text' style="height:200px;width:100%" placeholder="Add some info about yourself!" name='clientphone' >{{$user->aboutme}}</textarea></td>
                          </tr>
                        </table>
                        </center>
                        <center><button type='button' onclick='updateUser({{$user->id}})' id='buttonupdate' style="color:green" class='btn btn-light'>Save &#10004;</button></center></span>
                      </div>
                      <div class='' style=""><p>Do you want to change your password?</p>
                        ...
                        <button type='button' id='changepassword' style="" class='btn btn-light' onclick="changePassword({{$user->id}})">Change Password</button>
                      </div>
...
<!-- -------------------------------------End Update Account-------------------------------------- -->
 ...
 <div id="addVideoModal" class="modal" style="margin-top:55px">
   <span id="span" class="btn-close">&times;</span>
   <center><form class="" action="{{route('addvideo')}}" method="post" enctype="multipart/form-data">
     @csrf
      ...
      <table id="productstable"><tr>
        @foreach($userprod as $p)
          ...
          <td id='box_{{$p->id}}'><div class='box' id='box' style='display:relative'>
            ...
            <button type="button" onclick="deleteProduct({{$p->id}})" class="btn" id="deleteProduct_{{$p->id}}" style='color:red;font-weight:800;position:absolute;margin-top:-35px;padding:3px 5px;font-size:24px' name='remove'>&times</button>
            <div class='back'>
              <h1>{{$p->name}}</h1>
              <h5 style='color:#618685;'>{{$p->description}}</h5>
              @if($p->promotion>0)
              @php
                $pr=$p->price;
                $pro=$p->promotion;
              ...
              @endif
              <button type="button" onclick="returnProduct({{$p->id}})" style='margin-left:5px;padding:10px;border:0;background-color:transparent'><a class="nav-link">Edit</a></button></p>
              @if($commentsNb>0)
                <a href='/Comments/{{$p->id}}' class="nav-link" style=''>View comments</a>
              @endif
              </div>
            </div></td>
             ...
             <div id="main-content" class="bg-white border">
                 ...
                 <div class="d-flex my-4 flex-wrap">
                     <div class="box me-4 my-1 bg-light">
                         <img src="{{asset('css/icons/box.png')}}" style="width:55px;height:45px">
                         <div class="d-flex align-items-center mt-2">
                             <div class="tag">New Orders</div>
                             <div class="ms-auto number">{{$ordersNb}}</div>
                         </div>
                     </div>
                 ...
                 @foreach($orders as $o)
                     <div class="order my-3 bg-light {{$o->status}}" id="order_{{$o->id}}">
                         <div class="row">
                             <div class="col-lg-4">
                                 <div class="d-flex flex-column justify-content-between order-summary">
                                     <div class="d-flex align-items-center">
                                       <div class="text-uppercase">Product {{$o->name}}</div>
                                         <div class="green-label ms-auto text-uppercase">{{$o->cname}}</div>
                                     </div>
                                     <div class="fs-8">{{$o->created_at}}</div>
                                     <button class="btn btn-primary text-uppercase" value="{{$o->id}}">order info</button>
                                 </div>
                             </div>
                             <div class="col-lg-8">
                                 <div class="d-sm-flex align-items-sm-start justify-content-sm-between">
                                     ...
                                     @if($o->status=='pending'||$o->status=='confirmed')
                                       ...
                                       @endphp
                                       <div class="red-label ms-auto text-uppercase">@if($d1>$d2) Too Late @else @if($o->status=='confirmed')You have @else In @endif {{$days}} Days @endif</div>
                                      @endif
                                 </div>
                             ...
                             <div class="alert alert-success" id="alertConfirm_{{$o->id}}" style="display:none;margin-top:15px"></div>
                           </div>
                         </div>
                         @endforeach
                         <!-- Modal -->
                         @foreach($orders as $o)
                               <div class="modal" id="modalUser{{$o->id}}" tabindex="-1" aria-labelledby="exampleModalLabel"  aria-hidden="true">
                                 <div class="modal-dialog" style="">
                                   <div class="modal-content" style="width:500px">
                                     <div class="modal-header">
                                       <h5 class="modal-title" id="exampleModalLabel">Order Info</h5>
                                       <button type="button" class="btn-close" data-mdb-dismiss="modal" value="{{$o->id}}" aria-label="Close"></button>
                                     </div>
                                           ...
                                           <div class="col-md-8 col-lg-6 col-xl-4" style="width:450px">
                                             ...
                                             <div class="card-body pb-0">
                                               <div class="d-flex justify-content-between">
                                                 <div>
                                                   <p><a href="product/{{$o->productId}}" class="text-dark">{{$o->pname}}</a></p>
                                                   <p class="small text-muted">${{$o->price}}</p>
                                                   @if($o->promotion>0)
                                                    <p class="small text-muted">%{{$o->promotion}} Promotion</p>
                                                   @endif
                                                 </div>
                                                 <div>
                                                   <p><a class="text-dark">Qunatity: 0{{$o->quantity}}</a></p>
                                                 </div>
                                               </div>
                                             </div>
                                             <hr class="my-0" />
                                             <div class="card-body pb-0">
                                               <div class="d-flex justify-content-between">
                                                 ...
                                                 <p class="text-dark">${{$o->total_price}}</p>
                                               </div>
                                               @if($o->status!='Packed' && $o->status!='Delivered')
                                               <div class="d-flex justify-content-between">
                                                 <p><a class="text-dark">Delivery Date:</a></p>
                                                 <p class="text-dark">
                                                   <input type="date" class="form-control" id="delTime" name="" value="{{$o->expecting_delivery_date}}"></p>
                                               </div>@endif
                                             </div>
                                             <hr class="my-0" />
                                             <div class="card-body pb-0">
                                               <div class="d-flex justify-content-between">
                                                 ...
                                                 <p class="text-dark">{{$o->stock}} Items</p>
                                               </div>
                                             </div>
                                             <hr class="my-0" />
                                             <div class="card-body">
                                               <div class="d-flex justify-content-between align-items-center pb-2 mb-1">
                                                @if($o->status=='pending')
                                                  <button type="button" onclick="confirmOrder({{$o->id}})" class="btn btn-primary">Confirm</button>
                                                @elseif($o->status=='confirmed')
                                                  <button type="button" onclick="ready({{$o->id}})" class="btn btn-danger">Ready to Deliver</button>
                                                @endif
                                               </div>
                                             </div>
                                           </div>
          ...
          @foreach($userprod as $up)
              @if($up->soldno > '0')
                <div class='month-group'>
                 ...
                 <div class='bar' style='height:{{$up->soldno}}0px' title='{{$up->soldno}}'></div>{{$up->soldno}}
               </div>
               @endif
          @endforeach
